...
# Chinese text (to test Chinese tokenization performance)
chinese_text = r"""
自然语言处理是人工智能领域的一个重要分支，它涉及计算机科学、语言学和认知科学的交叉。

现代的大语言模型如GPT、BERT、T5等，在文本理解、生成和翻译等任务上取得了显著进展。

这些模型通过在大规模文本语料上进行预训练，学习到了丰富的语言知识和世界知识。

中文分词是中文自然语言处理的基础任务之一，对于提高模型在中文任务上的表现至关重要。

一个好的分词器应该能够准确识别词边界，理解上下文，并处理未登录词和新词。

BPE（Byte Pair Encoding）是一种常用的子词分词算法，它通过迭代合并最频繁的字符对来构建词汇表。

对于中英文混合的分词器，需要平衡两种语言的特点，确保在两种语言上都有良好的压缩比和分词效果。
""".strip()
